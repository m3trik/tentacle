...
def _state():
    ...
    try:
        with open(
            os.path.join(SANDBOX, "blendertk_script_output.json"), encoding="utf-8"
        ) as f:
            return json.load(f)
    except Exception:
        return {}
...
def _go():
    try:
        ...
        so.ScriptConsole.begin_capture()  # ≈ tentacle_startup.py — before `import tentacle`
        _ck(
            "begin_capture() tees stdout (UI-free, before any window/Qt use)",
            id(sys.stdout) != R["orig_stdout_id"],
        )

        from tentacle import (
            tcl_blender as tb,
        )  # greeting banner prints INTO the capture
        ...
        _ck(
            "restore() at launch with no saved state leaves the console hidden",
            not inst.is_open(),
        )
        ...
        _ck(
            "docking spawned NO new bpy.Window (true dock, not a separate window)",
            len(bpy.context.window_manager.windows) == R["win_before"],
        )
        _ck(
            "Info Log area docked into the main window's screen",
        ...
        _ck(
            "greeting banner printed BEFORE the console existed is in the console "
...
def _measure():
    try:
        ...
        _ck(
            "console is a true CHILD of the main GHOST window (WS_CHILD embed)",
        ...
        _ck(
            "resize strip: dock resolved a native edge pad (>=2px)",
            isinstance(pad, int) and pad >= 2,
            f"pad={pad}",
        )
        ...
        if expected:
            ...
            _ck(
                "console fills the content region minus the exposed resize strip (<=2px)",
                delta <= 2,
...
def _screen_center_of(hwnd, rect_client):
    ...
    return (
        org.x + rect_client[0] + rect_client[2] // 2,
...
def _hover_assert():
    ...
    try:
        ...
        if not is_fg:
            ...
            _ck(
                "hovering the console hands it OS keyboard focus (no click needed)",
                _focus_now() == child,
        ...
        if is_fg:
            ...
            _ck(
                "a REAL Ctrl+C while merely hovering copies the console's text "
                "(the point of the feature: shortcuts work on mouse-over)",
                "console verification print" in clip,
                f"clipboard[:60]={clip[:60]!r}",
            )
        ...
        vx, vy = _screen_center_of(
            R["_ghost"], BlenderWindow.region_client_rect(R["_ghost"], region)
        )
...
def _leave_assert():
    ...
    try:
        ...
        if R.get("focus_over_console") != R["_child"]:
            ...
            _ck(
                "leaving the console returns keyboard focus to Blender (its shortcuts "
...
def _hide():
    ...
    try:
        ...
        state = _state()
        _ck(
            "hide() undocks the Info Log area",
            len(_main_areas()) == R.get("areas_before", 0),
        )
        _ck(
            "hide() leaves stdout teed (capture persists)",
        ...
        _ck(
            "hide() persists the strip height (user resize survives sessions)",
...
def _reshow():
    try:
        ...
        _ck(
            "show() re-docks the Info Log area",
            len(_main_areas()) == R.get("areas_before", 0) + 1,
        )
        text = inst.widget.toPlainText() if inst.widget is not None else ""
        _ck(
            "earlier content ('console verification print') survived the hide->show cycle",
            "console verification print" in text,
        )
        _ck(
            "content printed WHILE hidden also shows once reshown (capture ran in the "
            "background)",
            "CONSOLE_DOCK_CHECK_HIDDEN_MARKER" in text,
        )
...
def _cold_restore():
    ...
    try:
        ...
        _ck(
            "teardown() (host-reload path) undocks the area",
            len(_main_areas()) == R.get("areas_before", 0),
        )
        _ck(
        ...
        _ck(
            "teardown() leaves the persisted flag alone (still visible=true)",
            _state().get("visible") is True,
        )

        so.ScriptConsole._instance = None  # "new process / reloaded module"
        inst2 = so.ScriptConsole.restore()
        for _ in range(20):
            app.processEvents()
        _ck("cold restore(): saved visible=true re-opens the console", inst2.is_open())
        _ck(
            "cold restore(): the Info Log area is re-docked",
            len(_main_areas()) == R.get("areas_before", 0) + 1,
        )
        _ck(
            "cold restore(): a fresh console widget was built", inst2.widget is not None
        )
        ...
        _ck(
            "cold restore(): new prints reach the restored console",
...
def _zorder_assert():
    ...
    try:
        ...
        hwnd2 = next(
            (int(h) for h, _t in _input.ghost_windows() if int(h) != ghost), None
        )
        R["_z_hwnd2"] = hwnd2
        ...
        if hwnd2:
            ...
            _ck(
                "a second Blender window covering the console rect OCCLUDES it "
                "(the embed stacks with its container, not above all windows)",
                root == int(hwnd2),
                f"root={root} hwnd2={hwnd2} ghost={ghost} child={R.get('_z_child')}",
            )

        so.ScriptConsole.hide()  # final cleanup
        _ck(
            "final hide() undocks the area and persists visible=false",
            len(_main_areas()) == R.get("areas_before", 0)
...
def _finish():
    ...
    )  # temp_tests artifacts clean up in teardown
