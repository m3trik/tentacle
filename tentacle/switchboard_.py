...
class Switchboard_(QUiLoader, StyleSheet):
	...
	def loadUi(self, file, widgets=None, parent=None):
		...
		#set properties
		ui.addWidgets = lambda u=ui, *args, **kwargs: self.addWidgets(u, *args, **kwargs)
	...
	def addWidgets(self, ui, widgets='all', inc=[], exc=[], filterByBaseType=False, **kwargs):
		...
		for w in Iter.makeList(widgets): #assure 'widgets' is a list.
			...
			w.isSynced = False
			w.__class__.method = property(lambda w: getattr(w.ui.slots, w.name, None))
	...
	def currentUi(self, ui) -> None:
		...
		print ('set current ui:', ui.name, self)
		self._currentUi = ui
		self._uiHistory.append(ui)
		print ('_uiHistory:', [u.name for u in self._uiHistory])
