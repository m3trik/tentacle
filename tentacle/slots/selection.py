...
class Selection(Slots):
    ...
    def __init__(self, *args, **kwargs):
        ...
        dh.ctxMenu.cmb006.setCurrentText(
            "Current Selection"
        )  # cmb.insertItem(cmb.currentIndex(), 'Current Selection') #insert item at current index.
        dh.ctxMenu.cmb006.popupStyle = "qmenu"
        dh.ctxMenu.cmb006.beforePopupShown.connect(
            self.cmb006
        )  # refresh the comboBox contents before showing it's popup.
        ...
        cmb001.returnPressed.connect(
            lambda m=cmb001.ctxMenu.lastActiveChild: getattr(self, m(name=1))()
        )  # connect to the last pressed child widget's corresponding method after return pressed. ie. self.lbl000 if cmb001.lbl000 was clicked last.
        cmb001.currentIndexChanged.connect(
            self.lbl005
        )  # select current set on index change.
        ...
        tb001.ctxMenu.chk018.stateChanged.connect(
            lambda state: self.sb.toggle_widgets(tb001.ctxMenu, setDisabled="chk011-18")
            if state
            else self.sb.toggle_widgets(tb001.ctxMenu, setEnabled="chk011-18")
