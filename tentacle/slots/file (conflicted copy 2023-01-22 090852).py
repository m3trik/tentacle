...
class File(Slots):
	...
	def __init__(self, *args, **kwargs):
		...
		recentFiles = [ptk.File.formatPath(f, 'name') for f in self.getRecentFiles()[:6] if f]
		# list000.setVisible(bool(recentFiles))
		recentFiles = ['fileA', 'fileB', 'fileC']
		list000.addItems(recentFiles)
		# self.sb.file_submenu.b001.setText(ptk.File.formatPath(mostRecentFile, 'name')) if mostRecentFile else self.sb.file_submenu.b001.setVisible(False)
