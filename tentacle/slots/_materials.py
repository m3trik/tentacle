...
Renaming a material is a **double-click on the combo's current item** (uitk's
``ComboBox.rename_on_double_click``), not a menu command. The two entries that used to do it
— a "Rename" label with a prefix/suffix affix option box, and "Rename (strip trailing ints &
_)" with its alpha/skip conflict toggle — are gone: the first is what the double-click now
is, and batch name surgery belongs where a table shows what it will hit (the Texture Path
Editor's Strip Trailing). The commit still forks per DCC, delegated to each slot's
``_rename_current(text)`` hook (Maya ``cmds.rename`` / Blender datablock assignment), which
returns the resulting name on success or a falsy value on failure.

Currently: the submenu's ``b003`` "Get + Select" one-shot — get the material off the current
