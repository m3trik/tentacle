...
class Create(Init):
	...
	def b000(self):
		...
		if type_ in ['Mesh', 'Editable Poly', 'Polygon', 'Editable Mesh', 'Editable Patch', 'NURBS']:
			if index==0: #cube:
				node = rt.Box(width=15, length=15, height=15, lengthsegs=1, widthsegs=1, heightsegs=1, name=name)
			elif index==1: #sphere:
				node = rt.Sphere(radius=5, segs=12)
			elif index==2: #cylinder:
				node = rt.Cylinder(radius=5, height=10, sides=12, heightsegs=1, capsegs=1, smooth=True, name=name)
			elif index==3: #plane:
				node = rt.Plane(width=5, length=5, widthsegs=1, lengthsegs=1, name=name)
			elif index==4: #circle:
				node = self.createCircle(axis=axis, numPoints=5, radius=5, mode=None, name=name)
			elif index==5: #Cone:
				node = rt.Cone(radius1=5, radius2=1, height=5, capsegs=1, heightsegs=1, sides=12, smooth=True, name=name)
			elif index==6: #Pyramid
				node = rt.Pyramid(width=5, depth=3, height=5, widthsegs=1, depthSegs=1, heightsegs=1, name=name)
			elif index==7: #Torus:
				node = rt.Torus(radius1=10, radius2=5, segs=5, name=name)
			elif index==8: #Pipe
				node = rt.Tube(radius1=5, radius2=8, height=25, sides=12, capSegs=1, hightSegs=1, name=name)
			elif index==9: #Soccer ball
				node = rt.GeoSphere(radius=5, segs=2, baseType=2, smooth=True, name=name)
		...
		if type_ in ['Hedra', 'Torus Knot', 'Chamfer Box', 'Chamfer Cylinder', 'Oil Tank', 'Capsule', 'Spindle', 'L-Extrusion', 'Gengon', 'C-Extrusion', 'RingWave', 'Hose', 'Prism']:
			if index==10: #Hedra
				rt.Hedra(family=0, scalep=100, scaleq=100, scaler=100, mapcoords=True, radius=13.2914, pos=[2.80033,-6.07454,0], isSelected=True, name=name)
			elif index==11: #Torus Knot
				rt.Torus_Knot(smooth=2, Base_Curve=0, segments=120, sides=12, radius=12.9131, radius2=2.82649, p=2, q=3, Eccentricity=1, Twist=0, Lumps=0, Lump_Height=0, Gen_UV=1, U_Tile=1, V_Tile=1, U_Offset=0, V_Offset=0, Warp_Height=0, Warp_Count=0, pos=[1.17498,-28.5641,0], isSelected=True, name=name)
			elif index==12: #Chamfer Box
				rt.ChamferBox(width=25.9508, fillet=2.08099, length=21.103, height=10.9843, pos=[57.3517,3.11597,0.005], isSelected=True, name=name)
			elif index==13: #Chamfer Cylinder
				rt.ChamferCyl(radius=9.90455, height=20.6785, fillet=0.792669, pos=[13.4798,-19.4707,0], isSelected=True, name=name)
			elif index==14: #Oil Tank
				rt.OilTank(radius=9.40818, Cap_Height=2.35204, height=24.5722, Blend=0, sides=12, Height_Segments=1, Smooth_On=1, Slice_On=0, Slice_From=0, Slice_To=0, mapcoords=1, pos=[-0.580549,-43.851,0], isSelected=True, name=name)
			elif index==15: #Capsule
				rt.Capsule(radius=8.8111, height=28.4398, heighttype=0, sides=12, heightsegs=1, smooth=True, sliceon=False, slicefrom=0, sliceto=0, mapcoords=True, pos=[-9.87687,-6.62625,0], isSelected=True, name=name)
			elif index==16: #Spindle
				rt.Spindle(radius=8.22467, Cap_Height=2.42833, height=21.3872, Blend=0, sides=12, Height_Segments=1, cap_segments=5, Smooth_On=1, Slice_On=0, Slice_From=0, Slice_To=0, mapcoords=1, pos=[-6.07937,-23.7136,0], isSelected=True, name=name)
			elif index==17: #L-Extrusion
				rt.L_Ext(Side_Length=10.0896, Front_Length=-5.57165, centerCreate=False, pos=[23.9174,-54.3066,0], isSelected=True, Front_Width=2.52126, Side_Width=2.52126, height=12.0807, name=name)
			elif index==18: #Gengon
				rt.Gengon(sides=5, radius=11.053, fillet=1.19902, height=20.8109, Side_Segments=1, Fillet_Segments=1, Height_Segments=1, mapcoords=1, pos=[-12.2299,13.1004,0], isSelected=True, name=name)
			elif index==19: #C-Extrusion
				rt.C_Ext(Front_Length=-7.54399, Back_Length=-7.54399, Side_Length=8.52505, centerCreate=False, pos=[-5.72067,-60.0124,0], isSelected=True, Front_Width=1.38667, Back_Width=1.38667, Side_Width=1.38667, height=22.6349, name=name)
			elif index==20: #Ringwave
				rt.RingWave(time_on=0, time_growing=9600, display_until=16000, repeats=2, max_diameter=8.17418, ring_width=2.37046, ring_segments=200, Outer_Edge_Breakup=False, Major_Cycles_Outer=1, Major_Cycle_Flux_Outer=0, Major_Cycle_Flux_Per_Outer=16000, Minor_Cycles_Outer=1, Minor_Cycle_Flux_Outer=0, Minor_Cycle_Flux_Per_Outer=-16000, Inner_Edge_Breakup=True, Major_Cycles_Inner=11, Major_Cycle_Flux_Inner=25, Major_Cycle_Flux_Per_Inner=19360, Minor_Cycles_Inner=29, Minor_Cycle_Flux_Inner=10, Minor_Cycle_Flux_Per_Inner=-4320, height=0, Height_Segs=1, Radius_Segs=1, Mapping_Coords=True, Smoothing=True, pos=[14.9087,-62.8923,0], isSelected=True, name=name)
			elif index==21: #Hose
				rt.Hose(End_Placement_Method=1, Hose_Height=26.8204, Segments_Along_Hose=45, Smooth_Spring=0, Renderable_Hose=1, Hose_Cross_Section_Type=0, Round_Hose_Diameter=10.3709, Round_Hose_Sides=8, Rectangular_Hose_Width=10.3709, Rectangular_Hose_Depth=10.3709, Rectangular_Hose_Fillet_Size=0, Rectangular_Hose_Fillet_Segs=0, Rectangular_Hose_Section_Rotation=0, D_Section_Hose_Width=10.3709, D_Section_Hose_Depth=10.3709, D_Section_Hose_Fillet_Size=0, D_Section_Hose_Fillet_Segs=0, D_Section_Hose_Round_Segs=4, D_Section_Hose_Section_Rotation=0, Generate_Mapping_Coordinates=1, Flex_Section_Enabled=1, Flex_Section_Start=10, Flex_Section_Stop=90, Flex_Cycle_Count=5, Flex_Section_Diameter=-20, Top_Tension=100, Bottom_Tension=100, pos=[-24.722,-55.7553,0], isSelected=True, name=name)
			elif index==22: #Prism
		...
		if selection:
			...
			if rt.subObjectLevel==1: #vertex
				...
				pos = rt.polyop.getVert(obj, vertex[0]) #Returns the position of the specified vertex.
			else:
				pos = obj.position
		else:
			pos = [0,0,0]
