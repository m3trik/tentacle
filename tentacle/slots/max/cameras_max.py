...
class Cameras_max(Cameras, SlotsMax):
    ...
    def clippingMenu(self):
        ...
        if not hasattr(self, "_clippingMenu"):
            ...
            self._clippingMenu.add(
                self.sb.Label,
                setText="Viewport Clip",
                setObjectName="lbl000",
                setToolTip="Toggle the clipping controls for the current viewport camera.",
            )
            self._clippingMenu.add(
                "QPushButton",
                setText="Auto Clip",
    ...
    def tree000(self, wItem=None, column=None):
        ...
        if not any(
            ...
            try:
                l = [
                    str(cam.name) for cam in rt.cameras if "Target" not in cam.name
                ]  # List scene Cameras
        ...
        if header == "Options":
            ...
            if text == "Toggle Safe Frames":
                maxEval(
                    'actionMan.executeAction 0 "219"'
                )  # Tools: Viewport Safeframes Toggle
