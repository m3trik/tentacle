...
class Edit_max(Edit, SlotsMax):
    def __init__(self, *args, **kwargs):
        ...
        if not ctx.contains_items:
            ...
            ctx.add(
                "QCheckBox",
                setText="Isolated Vertex",
                setObjectName="chk003",
                setChecked=True,
                setToolTip="Find isolated vertices within specified angle threshold.",
            )
            ctx.add(
                "QSpinBox",
    ...
    def tb001(self, state=None):
        ...
        if all_:
            if deformers:
                self.sb.message_box("Delete <hl>All</hl> History.")
                return
            else:
                self.sb.message_box("Delete <hl>All Non-Deformer</hl> History.")
                return
        else:
            if deformers:
                self.sb.message_box("Delete history on " + str(objects))
                return
            else:
                self.sb.message_box(
    ...
    def tb002(self, state=None):
        ...
        for obj in rt.selection:
            ...
            if level == 2:  # edges
                ...
                rt.polyop.setVertSelection(
                    obj, verts
                )  # set vertex selection to be used by meshCleanup

                obj.EditablePoly.remove(selLevel="Edge", flag=1)
                self.meshCleanup(
                    isolatedVerts=1, repair=1
                )  # if any isolated verts exist: delete them
    ...
    def getVertexVectors(self, obj, vertices):
        ...
        for vertex in vertices:
            ...
            if len(edges) == 2:
                ...
                try:
                    vector1 = rt.normalize(
                        rt.polyop.getVert(obj, edgeVerts[0]) - vertexPosition
                    )
                    vector2 = rt.normalize(
                        rt.polyop.getVert(obj, edgeVerts[1]) - vertexPosition
                    )
                except:
                    vector1 = rt.normalize(
                        rt.getVert(obj, edgeVerts[0]) - vertexPosition
                    )
                    vector2 = rt.normalize(
                        rt.getVert(obj, edgeVerts[1]) - vertexPosition
                    )
        ...
        for obj in rt.selection:
            if not rt.classof(obj) == rt.Editable_poly:
                print(
                    "Error: "
                    + obj.name
                    + " isn't an editable poly or nothing is selected."
                )
            ...
            if nGons:  # Convert N-Sided Faces To Quads
                ...
                if repair:
                    maxEval(
                        'macros.run "Modifiers" "QuadifyMeshMod"'
                    )  # add the quadify mesh modifier to the stack
