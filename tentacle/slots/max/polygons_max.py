...
class Polygons_max(Polygons, SlotsMax):
    ...
    def tb000(self, state=None):
        ...
        if selection:
            for n, obj in enumerate(selection):
                ...
                if (
                    rt.subObjectLevel == 1 and vertSelection > 1
    ...
    def tb005(self, state=None):
        ...
        for obj in rt.selection:
            ...
            if level == 4:  # faces
                ...
                if rt.queryBox(
                    "Detach as Element?", title="Detach"
                ):  # detach as element
    ...
    def detachElement(self, obj):
        ...
        if (
            obj[4] == rt.Editable_Poly and obj[7]
        ):  # or obj[6] == "Shape" or obj[6] == "Geometry"
            rename = obj[0].name
            rename += "_ele"
            ...
            while (rt.polyOp.getNumFaces(obj[0])) > 0:
                ...
                rt.polyOp.detachFaces(
                    obj[0], elementToDetach, delete=True, asNode=True, name=rename
                )
            rt.delete(obj[0])
