...
class Selection_max(Selection, SlotsMax):
            ...
            for obj in rt.selection:
                for i in cmb.items:
                    if index == cmb.items.index(i):
                        obj.convertSelection(
                            "CurrentLevel", i
                        )  # Convert current selection to index of string i
                        # rt.setSelectionLevel(obj, i) #Change component mode to i
                        rt.subObjectLevel = cmb.items.index(
                            i
    ...
    def cmb005(self, index=-1):
        ...
        if index > 0:
            ...
            if text == "Angle":
                ...
                )  # dR_DoCmd("selConstraintEdgeLoop");
            elif text == "Edge Ring":
                mel.eval(
                ...
                )  # dR_DoCmd("selConstraintUVEdgeLoop");
        else:
    ...
    def creatNewSelectionSet(self, name=None):
        ...
        if rt.isValidObj(name):  # obj!=rt.undefined
            self.sb.message_box("Set with name <hl>{}</hl> already exists.".format(name))
            return
            ...
            if sel:
                ...
                try:  # object level set.
                    ...
                    set_array = self.getSetArray(
                        rt.selection[0], rt.subObjectLevel
                    )  # ie. rt.selection[0].faces
                    set_array[
                        name
                    ] = sel  # create a sub-object level set for the selected currently selected components.
            else:
    ...
    def modifySet(self, name):
        ...
        if sel:
            ...
            try:  # object level set.
                set_.name = (
                    newName  # rename the old set with the comboBoxes current text.
                )
                rt.selectionSets[name] = sel  # create a standard object set.
            except IndexError:  # sub-object level set.
                set_array = self.getSet(name, "set_array")
    ...
    def getSelectionSets(self, objects=[], includeEmptySets=True):
        ...
        for level in [
            ...
            2,
            4,
        ]:  # range(5) omitting 'borders' because of attribute error: <obj.borders>
            for obj in objects:
    ...
    def selectionSets(self, obj=None, level=None):
        ...
        try:
            if obj is None:  # empty sets
                ...
                sets = {
                    s.name: s
                    for n, s in enumerate(rt.selectionSets, 1)
                    for i in range(1, rt.getNamedSelSetItemCount(n) + 1)
                    if rt.getNamedSelSetItem(n, i) == obj
                }
            elif level == 1:  # verts
            ...
            AttributeError
        ):  # skip if the obj type doesn't have an attribute of the given level: ie. object has no attribute 'selSetNames' (ex. non-editable mesh)
            sets = {}
