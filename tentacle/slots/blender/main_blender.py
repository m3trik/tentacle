...
# 						lambda value, widget=w, node=node: self.set_node_attributes(node, {attr:w.value()}))

# 			[w.valueChanged.connect(
# 				lambda value, widget=w, node=node: self.set_node_attributes(node, {widget.prefix().rstrip(': '):value}))
