...
class Duplicate_blender(Duplicate, SlotsBlender):
        ...
        if self.duplicate_radial_ui.chk015.isChecked():
            ...
            if selection:
                ...
                for object_ in selection:
                    ...
                    x = y = z = 0
                    if self.duplicate_radial_ui.chk012.isChecked():
                        x = angle
                    if self.duplicate_radial_ui.chk013.isChecked():
                        y = angle
                    if self.duplicate_radial_ui.chk014.isChecked():
                        z = angle
    ...
    def chk016(self, create=False):
        ...
        if self.duplicate_linear_ui.chk016.isChecked():
            ...
            transXYZ = [
                float(self.duplicate_linear_ui.s002.value()),
                float(self.duplicate_linear_ui.s003.value()),
                float(self.duplicate_linear_ui.s004.value()),
            ]
            rotXYZ = [
                float(self.duplicate_linear_ui.s007.value()),
                float(self.duplicate_linear_ui.s008.value()),
                float(self.duplicate_linear_ui.s009.value()),
            ]
            translateToComponent = self.duplicate_linear_ui.chk007.isChecked()
            alignToNormal = self.duplicate_linear_ui.chk008.isChecked()
            componentList = [
                self.duplicate_linear_ui.cmb001.itemText(i)
                for i in range(self.duplicate_linear_ui.cmb001.count())
            ]
    ...
    def convertToInstances(self, objects=[], leaf=False, append="_INST"):
        ...
        for obj in objects[1:]:
            ...
            pm.matchTransform(
                instance, obj, position=1, rotation=1, scale=0, pivots=1
