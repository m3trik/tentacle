...
class Normals_blender(Normals, SlotsBlender):
    ...
    def tb001(self, state=None):
        ...
        for obj in objects:
            ...
            allEdges = edges = pm.ls(
                pm.polyListComponentConversion(obj, toEdge=1), flatten=1
            )
    ...
    def getNormalVector(name=None):
        ...
        if type_ == "mesh":  # get face normals
            ...
            numFaces = pm.polyEvaluate(
                name, face=1
            )  # returns number of faces as an integer
            normals = []
            for n in range(0, numFaces):  # for (number of faces):
                array = pm.polyInfo(
                    "{0}[{1}]".format(name, n), faceNormals=1
                )  # get normal info from the rest of the object's faces
                string = " ".join(array)
                n.append(str(string))
