...
class File_blender(File, SlotsBlender):
    ...
    def cmb003(self, index=-1):
        ...
        if index > 0:  # hide then perform operation
            ...
            if index == 1:  # Import
                ...
                mel.eval(
                    "FBXUICallBack -1 editImportPresetInNewWindow fbx;"
                )  # Fbx Presets
            elif index == 4:  # Obj Import Presets
                mel.eval(
                    "FBXUICallBack -1 editImportPresetInNewWindow obj;"
                )  # Obj Presets
            cmb.setCurrentIndex(0)
    ...
    def cmb004(self, index=-1):
        ...
        if index > 0:  # hide then perform operation
            ...
            if index == 1:  # Export selection
                ...
                )  # OneClickMenuExecute ("3ds Max", "SendAsNewScene"); doMaxFlow { "sendNew","perspShape","1" };
            elif index == 6:  # Send to 3dsMax: Update Current
                mel.eval(
                    "UpdateCurrentScene3dsMax;"
                )  # OneClickMenuExecute ("3ds Max", "UpdateCurrentScene"); doMaxFlow { "update","perspShape","1" };
            elif index == 7:  # Send to 3dsMax: Add to Current
                mel.eval(
                    "AddToCurrentScene3dsMax;"
                )  # OneClickMenuExecute ("3ds Max", "AddToScene"); doMaxFlow { "add","perspShape","1" };
                ...
                mel.eval(
                    "FBXUICallBack -1 editExportPresetInNewWindow obj;"
                )  # Obj Presets
            cmb.setCurrentIndex(0)
