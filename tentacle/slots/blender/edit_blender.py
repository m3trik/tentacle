...
class Edit_blender(Edit, SlotsBlender):
    def __init__(self, *args, **kwargs):
        ...
        if not ctx.contains_items:
            ...
            ctx.chk013.toggled.connect(
                lambda state: ctx.s006.setEnabled(True if state else False)
            )
            ctx.chk014.toggled.connect(
                lambda state: ctx.s007.setEnabled(True if state else False)
            )
            ctx.chk015.toggled.connect(
                lambda state: ctx.s008.setEnabled(True if state else False)
            )
            ctx.chk022.stateChanged.connect(
                lambda state: self.sb.toggle_widgets(
                    ctx,
                    setDisabled="chk002-3,chk005,chk010-21,s006-8",
                    setEnabled="chk023",
                )
                if state
    ...
    def get_all_faces_on_axis(self, obj, axis="-x", localspace=False):
        ...
        if axis.startswith("-"):  # any([axis=="-x", axis=="-y", axis=="-z"]):
            return list(
                face
                for face in pm.filterExpand(obj + ".f[*]", sm=34)
        ...
        if nGons:  # N-Sided Faces
            if repair:  # Maya Bonus Tools: Convert N-Sided Faces To Quads
                try:
                    ...
                    print(
                        "Maya Bonus Tools: Convert N-Sided Faces To Quads not found. (bt_polyNSidedToQuad;)"
                    )
