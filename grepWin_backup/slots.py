...
class Slots(QtCore.QObject):
	...
	def hideMain(fn):
		...
		def wrapper(self, *args, **kwargs):
			...
			self.main.hide() #Get the state of the widget in the current ui and set any widgets (having the methods name) in child or parent ui's accordingly.
		return wrapper
	...
	def progressBar(self):
		...
		if not hasattr(self, '_progressBar'):
			...
			self._progressBar = ProgressBar(self.main)
	...
	def messageBox(self, string, location='topMiddle', timeout=1):
		...
		if not hasattr(self, '_messageBox'):
			...
			self._messageBox = MessageBox(self.main.parent())
